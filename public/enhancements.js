...
export function makeEnhancements(ctx){
 ...
 async function readImage(file,kind='banner'){
  ...
  if(file.type==='image/gif'){
   if(file.size>8*1024*1024)throw Error('O GIF do banner deve ter até 8 MB.');
 ...
 async function plusStore(ownedOnly=false){
  ...
  ctx.modal.querySelectorAll('[data-combo]').forEach(b=>b.onclick=ctx.action(async()=>{const combo=PLUS_COMBOS.find(c=>c.id===b.dataset.combo);if(inventory.includes(combo.id)){await ctx.api('/appearance',Object.fromEntries(combo.items.map(i=>[PLUS_ITEMS[i].slot,i])),'PATCH');await ctx.refresh();ctx.toast('Combo equipado. Todos já podem ver no seu perfil.');return appearance();}if(combo.quest)return quest();ctx.showModal('Adquirir combo',`<p>Tem certeza que deseja adquirir <strong>${h(combo.name)}</strong>?</p><p>Este combo inclui ${combo.items.length} ${combo.items.length===1?'item':'itens'}:</p><ul>${combo.items.map(i=>`<li>${h(PLUS_ITEMS[i].name)}</li>`).join('')}</ul><p><strong>Grátis · sem cobrança.</strong></p><button type="button" class="primary" id="confirm-combo">Adquirir ${h(combo.name)}</button>`);$('#confirm-combo').onclick=ctx.action(async()=>{$('#confirm-combo').disabled=true;try{await ctx.api('/plus/acquire',{combo:combo.id});await plusStore(true);ctx.toast('Combo adicionado ao seu inventário.');}catch(e){$('#confirm-combo').disabled=false;throw e;}});}));
 }
 async function quest(){
  const questions=[['Como você adiciona um amigo?',['Pelo nome do servidor','Pelo nome de usuário','Pela cor do perfil']],['Onde ficam os itens que você adquiriu?',['Nas chamadas','Na busca de mensagens','No inventário do Ilia Plus']],['O que faz o botão de mutar na chamada?',['Desliga seu microfone','Apaga a conversa','Expulsa os membros']]];
  ctx.showModal('Missão: Pluma lunar', '<div class="quest-reward"><span class="nameplate plate-moonfeather">'+h(ctx.user().display)+'</span></div><p>Acerte as 3 perguntas para ganhar esta placa. Pode tentar quantas vezes quiser.</p>'+questions.map((q,i)=>'<fieldset class="quest-question"><legend>'+ (i+1)+'. '+h(q[0])+'</legend>'+q[1].map((answer,j)=>'<label><input type="radio" name="q'+i+'" value="'+j+'"> '+h(answer)+'</label>').join('')+'</fieldset>').join('')+'<p id="quiz-result" role="status"></p><button type="button" class="primary" id="quiz-submit">Concluir missão</button>');
  $('#quiz-submit').onclick=ctx.action(async()=>{const values=new FormData($('#modal-form'));const answers=questions.map((_,i)=>values.has('q'+i)?Number(values.get('q'+i)):-1);if(answers.includes(-1))throw Error('Responda às três perguntas.');const button=$('#quiz-submit');button.disabled=true;try{const result=await ctx.api('/plus/quest',{answers});if(!result.completed){$('#quiz-result').textContent='Você acertou '+result.score+' de 3. Ajuste suas respostas e tente novamente.';return;}await plusStore(true);ctx.toast('Missão concluída! Pluma lunar está no seu inventário. Clique em Equipar combo.');}finally{button.disabled=false;}});
 }
